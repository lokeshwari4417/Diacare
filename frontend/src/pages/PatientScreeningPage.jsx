...
export default function PatientScreeningPage() {
  ...
  const [method, setMethod] = useState('manual') // 'manual' | 'scan'
  ...
  const onExtracted = (res) => {
    setValues({
      pregnancies: res.extracted.pregnancies,
      glucose: res.extracted.glucose,
      blood_pressure: res.extracted.blood_pressure,
      skin_thickness: res.extracted.skin_thickness,
      insulin: res.extracted.insulin,
      bmi: res.extracted.bmi,
      diabetes_pedigree_function: res.extracted.diabetes_pedigree_function,
      age: res.extracted.age,
    })
    ...
    >
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted text-sm mt-1">{t('screening.subtitle')}</p>
        </div>
      ...
      <div className="glass-card">
        <div className="flex gap-2 mb-6 bg-primary-light/50 rounded-xl p-1 w-fit">
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${method === 'manual' ? 'bg-white shadow-sm text-primary' : 'text-muted hover:text-ink'}`}
          >
            {t('screening.manualTab')}
          </button>
          <button
            onClick={() => setMethod('scan')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${method === 'scan' ? 'bg-white shadow-sm text-primary' : 'text-muted hover:text-ink'}`}
          >
            {t('screening.scanTab')}
          </button>
        </div>
        ...
        {(method === 'manual' || scanned) && (
          <div className={method === 'scan' ? 'mt-6' : ''}>
            ...
            <RiskForm
              ...
              confidence={scanned ? confidence : undefined}
            />
          </div>
        )}
